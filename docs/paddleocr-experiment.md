...
- `慈愛微型`、`新扶愛`、`美保發`、`美利保`、`臻美利` 商品 DM 樣本已用 Docker 批次煙霧測試。
...
- `慈愛微型`：單一位數清單序號類 OCR 噪音需被壓掉，保留 Page 3 內容變更與 Page 4 footer 版號。
- `新扶愛`：電話、標點、空白或同數字段落重排不應因含有 `0800`、`40歲`、`310元`、`15足歲` 就進正式差異；保留真條文區塊變更與 footer。
- `美利保`：金額 OCR 若出現 `i/I/O` 夾在數字中，需用高倍率局部 OCR 重讀，避免顯示 `447,6i12` 這類髒值。
...
- **單一位數保守處理**：`2 -> 3` 這種一位數局部 OCR 若沒有強上下文，先視為不可靠，避免清單序號或版面位移誤報。
- **數字髒字元重讀**：只有當局部 OCR 在數字中混入 `i/I/O` 等疑似誤讀時，才對同一小框追加一次高倍率 OCR；不是全頁重跑。
