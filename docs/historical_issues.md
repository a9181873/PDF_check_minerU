...
### 解法：長度對齊閘 `_aligned_length` 取代相似度／純 digit-master（2026-05-25，已過 5 樣本容器回歸）
先**量測**（容器內 recall ON、CPU、本機 5 對 image PDF 真實 OCR；臨時 runner 依 `recall-regression-runbook.md` 步驟 1 自建、跑完即刪），dump 每個 matched/recon 配對的 sim / 長度 / 位數 / 判決，逐筆對 ground truth。決定性發現：**真正的判別訊號是「兩塊長度是否相近」，不是文字相似度**——

| 樣本 | 配對 | sim | lenO/lenN | len-ratio | 真相 | 應 |
|---|---|---|---|---|---|---|
| 臻美利 | 註1 ×3 | 0.68 | 296/154 | **0.52** | 重切噪音（新塊在 註2 前被截斷；可見差異只有 3.90↔3,90＋括號） | 抑制 |
| 美鑫 | 利率/註1/註5 ×4 | 0.96–0.99 | 72/72、76/76 | **1.0** | 真改 3.90→4.00、2.25→2.50（原地換數字） | 報 |
| 新保安心 | 文號 | **0.667** | 38/37 | **0.97** | **真改**（民國112→113年、文號 1110152342→11304921175） | 報 |
| 新保安心 | reflow ×3 | 0.28–0.65 | 165/55、64/50、52/65 | 0.33–0.80 | 段落 reflow/誤配 | 抑制 |

關鍵反例：**新保安心 文號是真改卻只有 sim 0.667**（短字串裡多個數字一起變，相似度被拉低）——文件先前規劃的「相似度 < 門檻才報」會**反向誤殺**它。而 len-ratio：真改全 ≥0.97、噪音全 ≤0.80，中間 [0.80, 0.97] 有寬裕間隙。故改用長度對齊。

- **`_aligned_length(a, b)`**：兩正規化塊長度相近視為「同一內容窗格」→ `longer - shorter <= 2`（短塊 OCR 抖動容忍）**或** `shorter/longer >= _ALIGNED_LEN_RATIO`（0.90）。
- 套用於 **matched 路徑（`_recall_block_item`）與 reconcile 配對路徑**：位數有變**且**長度對齊才報；長度不齊＝重切/reflow，位數差是窗格假象（真改另由 ADD/reconcile 的 `_digits_covered` 新數字守門召回，美鑫已證雙重覆蓋）。取代先前的 `_NOISE_SIM` 與 matched 重切 containment 閘。
- 單元測試：移除舊「重切＋真數字仍報（matched 路徑）」案（其保證改由 cross-page ADD 路徑的既有測試覆蓋），新增「長度不齊重切→抑制」與「低相似同長真改（文號）→報」兩例。後端全套件 **host 50 passed**。

**5 樣本容器端到端回歸（recall ON、CPU、`diff_positioned_paragraphs` 真實輸出）全過**：
- 臻美利 7→**3**：3 筆 註1 重切 FP **消**、p6 公式塊亂碼 DELETED 亦消（見下）；僅剩 3 筆 真 註3 新增。
- 美鑫 **7**：利率/註1/註5×2 NUMBER_MODIFIED＋祝壽 1,448,275 等 ADDED 全留，**0 FP**。
- 新保安心 5→**2**：文號 MODIFIED＋監護宣告新增條款留；3 筆 reflow FP **消**。
- 美保發 **0**、鳳守愛 **0**：乾淨（頁尾走像素護欄、鳳守愛 24→28 嵌在點陣圖 OCR 不取）。
- **公式塊亂碼一併修**：新增 `_MATH_FORMULA_NOISE_RE`（`∑∫∏√∮`）於 `_is_reliable_ocr_text`——含這些數學運算子＝公式片段，MinerU 必讀成亂碼、recall 無法可靠 diff，一律判不可靠。臻美利 p6「C V + ∑E n d(1+i)…」DELETED FP 消。此類密集公式／費率格網本就由像素路徑＋`suppressed_count` 橫幅負責。其餘 4 樣本內容無此類符號→行為不變（已單樣本複跑 臻美利確認 FORMULA PRESENT=False、3 筆 真 註3 仍在）。
- **預設旗標**：回歸全綠、無殘留 FP。經確認 `ENABLE_IMAGE_TEXT_RECALL` **維持預設 false**——上線為產品決策，待多看幾組或人工 UI 驗證後再定；compose 預設 false 不變。
